...
!
  ...
  if (useGPU) then
    ...
    call check_alloc_GPU_f("elpa_cholesky: tmp1_dev", 309,  successGPU)

    successGPU = gpu_memset(tmp1_dev, 0, nblk*nblk*size_of_datatype)
    call check_memcpy_GPU_f("elpa_cholesky: memset tmp1_dev", 320,  successGPU)

    successGPU = gpu_malloc(tmp2_dev, nblk*nblk*size_of_datatype)
    call check_alloc_GPU_f("elpa_cholesky: tmp2_dev", 324,  successGPU)

    successGPU = gpu_memset(tmp2_dev, 0, nblk*nblk*size_of_datatype)
    call check_memcpy_GPU_f("elpa_cholesky: memset tmp2_dev", 335,  successGPU)

    successGPU = gpu_malloc(tmatc_dev, l_cols*nblk*size_of_datatype)
    call check_alloc_GPU_f("elpa_cholesky: tmatc_dev", 339,  successGPU)

    successGPU = gpu_memset(tmatc_dev, 0, l_cols*nblk*size_of_datatype)
    call check_memcpy_GPU_f("elpa_cholesky: memset tmatc_dev", 350,  successGPU)

    successGPU = gpu_malloc(tmatr_dev, l_rows*nblk*size_of_datatype)
    call check_alloc_GPU_f("elpa_cholesky: tmatr_dev", 354,  successGPU)

    successGPU = gpu_memset(tmatr_dev, 0, l_rows*nblk*size_of_datatype)
    call check_memcpy_GPU_f("elpa_cholesky: memset tmatr_dev", 365,  successGPU)

    successGPU = gpu_malloc(a_dev, matrixRows*matrixCols*size_of_datatype)
    call check_alloc_GPU_f("elpa_cholesky: a_dev", 370,  successGPU)
  endif ! useGPU

  allocate(tmp1(nblk*nblk), stat=istat, errmsg=errorMessage)
  call check_allocate_f("elpa_cholesky: tmp1", 391,  istat,  errorMessage)

  allocate(tmp2(nblk,nblk), stat=istat, errmsg=errorMessage)
  call check_allocate_f("elpa_cholesky: tmp2", 394,  istat,  errorMessage)
  ...
  call check_allocate_f("elpa_cholesky: tmatr", 410,  istat,  errorMessage)

  allocate(tmatc(l_cols,nblk), stat=istat, errmsg=errorMessage)
  call check_allocate_f("elpa_cholesky: tmatc", 413,  istat,  errorMessage)
  ...
  if (useGPU) then
    ...
    call check_memcpy_GPU_f("elpa_cholesky 1: memcpy a-> a_dev", 451,  successGPU)
  endif
    ...
    if (n+nblk > na) then
      ...
      if (useGPU) then
        if (my_prow==prow(n, nblk, np_rows) .and. my_pcol==pcol(n, nblk, np_cols)) then
          ...
          call check_memcpy_GPU_f("elpa_cholesky: memcpy a_dev-> a", 505,  successGPU)
          ...
          successGPU = gpu_memcpy(a_dev, int(loc(a(1,1)),kind=c_intptr_t), &
                       matrixRows*matrixCols* size_of_datatype, gpuMemcpyHostToDevice)
          call check_memcpy_GPU_f("elpa_cholesky: memcpy a_dev-> a", 528,  successGPU)
    ...
    if (my_prow==prow(n, nblk, np_rows)) then
      ...
      if (my_pcol==pcol(n, nblk, np_cols)) then
        ...
        if (useGPU) then
          ...
          successGPU = gpu_memcpy(int(loc(a(1,1)),kind=c_intptr_t), a_dev,  &
                       matrixRows*matrixCols* size_of_datatype, gpuMemcpyDeviceToHost)
          call check_memcpy_GPU_f("elpa_cholesky: memcpy a_dev-> a", 676,  successGPU)
          ...
          successGPU = gpu_memcpy(a_dev, int(loc(a(1,1)),kind=c_intptr_t), &
                       matrixRows*matrixCols* size_of_datatype, gpuMemcpyHostToDevice)
          call check_memcpy_GPU_f("elpa_cholesky: memcpy a_dev-> a", 699,  successGPU)
          call obj%timer%stop("blas")
...
!#ifndef WITH_CUDA_AWARE_MPI
      if (useGPU) then
        ...
        call check_memcpy_GPU_f("elpa_cholesky: tmp1_dev to tmp1", 846,  successGPU)
      endif
!#endif
...
!      call check_memcpy_GPU_f("cholesky: device_synchronize", 870,  successGPU)
...
!#ifndef WITH_CUDA_AWARE_MPI
      if (useGPU) then
        ...
        call check_memcpy_GPU_f("elpa_cholesky: tmp1 to tmp1_dev", 906,  successGPU)
      endif
!#endif
    ...
    if (useGPU) then
      if (l_cols-l_colx+1 > 0) then
        ...
        successGPU = gpu_memcpy(int(loc(tmatc),kind=c_intptr_t), tmatc_dev, num, &
                              gpuMemcpyDeviceToHost)
        call check_memcpy_GPU_f("elpa_cholesky: tmatc_dev to tmatc", 1005,  successGPU)
      endif
    endif
!#endif
...
!    if (wantDebug) call obj%timer%start("cuda_aware_device_synchronize")
!    successGPU = gpu_devicesynchronize()
!    call check_memcpy_GPU_f("cholesky: device_synchronize", 1031,  successGPU)
!    if (wantDebug) call obj%timer%stop("cuda_aware_device_synchronize")
!
!    do i=1,nblk
!      call obj%timer%start("mpi_cuda_communication")
    ...
    if (useGPU) then
      !if (l_cols-l_colx+1 > 0) then
        ...
        successGPU = gpu_memcpy(tmatc_dev, int(loc(tmatc),kind=c_intptr_t), num, &
                              gpuMemcpyHostToDevice)
        call check_memcpy_GPU_f("elpa_cholesky: tmatc to tmatc_dev", 1068,  successGPU)
      !endif
    endif
!#endif
...
!#if !defined(1) || (defined(1) && defined(WITH_CUDA_AWARE_MPI))
      ...
      successGPU = gpu_memcpy(int(loc(tmatr),kind=c_intptr_t), tmatr_dev, num, &
                              gpuMemcpyDeviceToHost)
      call check_memcpy_GPU_f("elpa_cholesky: tmatr_dev to tmatr", 1125,  successGPU)
    endif
    ...
    if (useGPU) then
      ...
      successGPU = gpu_memcpy(tmatr_dev, int(loc(tmatr),kind=c_intptr_t), num, &
                              gpuMemcpyHostToDevice)
      call check_memcpy_GPU_f("elpa_cholesky: tmat to tmatr_dev", 1160,  successGPU)
    endif
  ...
  if (useGPU) then
    ...
    call check_dealloc_GPU_f("elpa_cholesky: tmp1_dev", 1211,  successGPU)

    successGPU = gpu_free(tmp2_dev)
    call check_dealloc_GPU_f("elpa_cholesky: tmp1_dev", 1214,  successGPU)

    successGPU = gpu_free(tmatc_dev)
    call check_dealloc_GPU_f("elpa_cholesky: tmatc_dev", 1217,  successGPU)

    successGPU = gpu_free(tmatr_dev)
    call check_dealloc_GPU_f("elpa_cholesky: tmatr_dev", 1220,  successGPU)
  endif


  deallocate(tmp1, tmp2, tmatr, tmatc, stat=istat, errmsg=errorMessage)
  call check_deallocate_f("elpa_cholesky: tmp1, tmp2, tmatr, tmatc", 1237,  istat,  errorMessage)
  ...
  if (useGPU) then
    successGPU = gpu_memcpy(int(loc(a(1,1)),kind=c_intptr_t), a_dev,  &
                     matrixRows*matrixCols* size_of_datatype, gpuMemcpyDeviceToHost)
    call check_memcpy_GPU_f("elpa_cholesky: memcpy 2 a-> a_dev", 1261,  successGPU)
  endif
  ...
  if (useGPU) then
    ...
    !successGPU = gpu_memcpy(int(loc(a(1,1)),kind=c_intptr_t), a_dev,  &
    !                   matrixRows*matrixCols* size_of_datatype, gpuMemcpyDeviceToHost)
    !call check_memcpy_GPU_f("elpa_cholesky: memcpy a-> d_dev", 1310,  successGPU)

    successGPU = gpu_free(a_dev)
    call check_dealloc_GPU_f("elpa_cholesky: a_dev", 1313,  successGPU)
...
!
  ...
  if (useGPU) then
    ...
    call check_allocate_f("elpa_cholesky: a_tmp", 380,  istat,  errorMessage)
  endif ! useGPU
      ...
      if (useGPU) then
        if (my_prow==prow(n, nblk, np_rows) .and. my_pcol==pcol(n, nblk, np_cols)) then
          ...
          successGPU = gpu_memcpy(int(loc(a_tmp(1,1)),kind=c_intptr_t), a_dev,  &
                       matrixRows*matrixCols* size_of_datatype, gpuMemcpyDeviceToHost)
          call check_memcpy_GPU_f("elpa_cholesky: memcpy a_dev-> a_tmp", 564,  successGPU)
          ...
          successGPU = gpu_memcpy(a_dev, int(loc(a_tmp(1,1)),kind=c_intptr_t), &
                       matrixRows*matrixCols* size_of_datatype, gpuMemcpyHostToDevice)
          call check_memcpy_GPU_f("elpa_cholesky: memcpy a_dev-> a_tmp", 587,  successGPU)
          call obj%timer%stop("blas")
    ...
    if (my_prow==prow(n, nblk, np_rows)) then
      ...
      if (my_pcol==pcol(n, nblk, np_cols)) then
        ...
        if (useGPU) then
          ...
          successGPU = gpu_memcpy(int(loc(a_tmp(1,1)),kind=c_intptr_t), a_dev,  &
                       matrixRows*matrixCols* size_of_datatype, gpuMemcpyDeviceToHost)
          call check_memcpy_GPU_f("elpa_cholesky: memcpy a_dev-> a_tmp", 735,  successGPU)
          ...
          successGPU = gpu_memcpy(a_dev, int(loc(a_tmp(1,1)),kind=c_intptr_t), &
                       matrixRows*matrixCols* size_of_datatype, gpuMemcpyHostToDevice)
          call check_memcpy_GPU_f("elpa_cholesky: memcpy a_tmp-> a_dev", 758,  successGPU)
          call obj%timer%stop("blas")
...
!#if !defined(1) || (defined(1) && defined(WITH_CUDA_AWARE_MPI))
  ...
  if (useGPU) then
    successGPU = gpu_memcpy(int(loc(a_tmp(1,1)),kind=c_intptr_t), a_dev,  &
                     matrixRows*matrixCols* size_of_datatype, gpuMemcpyDeviceToHost)
    call check_memcpy_GPU_f("elpa_cholesky: memcpy 2 a-> a_dev", 1283,  successGPU)
  endif
  ! Set the lower triangle to 0, it contains garbage (form the above matrix multiplications)
  ...
  !endif ! useGPU
    ...
    successGPU = gpu_memcpy(a_dev, int(loc(a_tmp(1,1)),kind=c_intptr_t), &
                       matrixRows*matrixCols* size_of_datatype, gpuMemcpyHostToDevice)
    call check_memcpy_GPU_f("elpa_cholesky: memcpy a_tmp-> a_dev", 1340,  successGPU)



    deallocate(a_tmp, stat=istat, errmsg=errorMessage)
    call check_deallocate_f("elpa_cholesky: a_tmp", 1350,  istat,  errorMessage)
