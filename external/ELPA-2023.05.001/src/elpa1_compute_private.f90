...
!-------------------------------------------------------------------------------
  ...
  call check_allocate_f("elpa_transpose_vectors: aux", 205,  istat,  errorMessage)
  do n = 0, lcm_s_t-1
...
!            k = k+nblk
  ...
  call check_deallocate_f("elpa_transpose_vectors: aux", 304,  istat,  errorMessage)
...
!
  ...
  if (useGPU) then
    ...
    call check_alloc_GPU_f("tridiag: a_dev", 549,  successGPU)


    successGPU = gpu_memcpy(a_dev, int(loc(a_mat(1,1)),kind=c_intptr_t), &
                              num, gpuMemcpyHostToDevice)
    call check_memcpy_GPU_f("tridiag: a_dev", 569,  successGPU)
  ...
  do istep = na, nblockEnd ,-1
    ...
    if (my_pcol == pcol(istep, nblk, np_cols)) then
      ...
      if (useGPU) then
        ...
        call check_memcpy_GPU_f("tridiag a_dev 1", 689,  successGPU)
      else ! useGPU
...
!          call prmat(na, useGpu, a_mat, a_dev, matrixRows, matrixCols, nblk, my_prow, my_pcol, np_rows, np_cols, "HH vec stored",  na - istep + 1)
    ...
    if (l_rows > 0 .and. l_cols> 0 ) then
     if (useGPU) then
       ...
       call check_memcpy_GPU_f("tridiag: v_col_dev", 1079,  successGPU)


       successGPU = gpu_memcpy(v_row_dev, int(loc(v_row(1)),kind=c_intptr_t), &
                                 l_rows * size_of_datatype, gpuMemcpyHostToDevice)
       call check_memcpy_GPU_f("tridiag: v_row_dev", 1093,  successGPU)
     endif ! useGPU
...
!                 endif
            ...
            call check_memcpy_GPU_f("tridiag: u_col_dev 1", 1272,  successGPU)

            successGPU = gpu_memcpy(int(loc(u_row(1)),kind=c_intptr_t), &
                        u_row_dev, l_rows * size_of_datatype, gpuMemcpyDeviceToHost)
            call check_memcpy_GPU_f("tridiag: u_row_dev 1", 1285,  successGPU)
          endif ! useGPU
       ...
       if (n_stored_vecs == max_stored_uv .or. istep == 3) then
         ...
         if (useGPU) then
           ...
           call check_memcpy_GPU_f("tridiag: uv_stored_rows_dev", 1449,  successGPU)

           successGPU = gpu_memcpy(uv_stored_cols_dev, int(loc(uv_stored_cols(1,1)),kind=c_intptr_t), &
                                     max_local_cols * 2 * max_stored_uv *          &
                                     size_of_datatype, gpuMemcpyHostToDevice)
           call check_memcpy_GPU_f("tridiag: uv_stored_cols_dev", 1463,  successGPU)
         endif
       ...
       if (my_prow == prow(istep-1, nblk, np_rows) .and. my_pcol == pcol(istep-1, nblk, np_cols)) then
         if (useGPU) then
           !a_mat(l_rows,l_cols) = a_dev(l_rows,l_cols)
            ...
            call check_memcpy_GPU_f("tridiag: a_dev 3", 1539,  successGPU)
         endif ! useGPU
         ...
         if (useGPU) then
           ...
           !call check_memcpy_GPU_f("tridiag: a_dev 4a5a", 1561,  successGPU)
           ...
           call check_memcpy_GPU_f("tridiag: a_dev 4", 1574,  successGPU)
         endif ! useGPU
  ...
  if (my_prow==prow(1, nblk, np_rows) .and. my_pcol==pcol(2, nblk, np_cols)) then
    if (useGPU) then
      successGPU = gpu_memcpy(int(loc(e_vec(1)),kind=c_intptr_t), a_dev + (matrixRows * (l_cols - 1)) * size_of_datatype, &
                              1 * size_of_datatype, gpuMemcpyDeviceToHost)
      call check_memcpy_GPU_f("tridiag: a_dev 7", 1683,  successGPU)
    else !useGPU
  ...
  if (my_prow==prow(1, nblk, np_rows) .and. my_pcol==pcol(1, nblk, np_cols)) then
    if(useGPU) then
      ...
      call check_memcpy_GPU_f("tridiag: a_dev 8", 1704,  successGPU)
    else !useGPU
      if (isSkewsymmetric) then
        d_vec(1) = 0.0_rk
      else
  ...
  call check_deallocate_f("tridiag: tmp", 1717,  istat,  errorMessage)

  if (useGPU) then

    ! todo: should we leave a_mat on the device for further use?
    successGPU = gpu_free(a_dev)
    call check_dealloc_GPU_f("tridiag: a_dev 9", 1733,  successGPU)

    successGPU = gpu_free(v_row_dev)
    call check_dealloc_GPU_f("tridiag: v_row_dev", 1736,  successGPU)

    successGPU = gpu_free(u_row_dev)
    call check_dealloc_GPU_f("tridiag: (u_row_dev", 1739,  successGPU)

    successGPU = gpu_free(v_col_dev)
    call check_dealloc_GPU_f("tridiag: v_col_dev", 1742,  successGPU)

    successGPU = gpu_free(u_col_dev)
    call check_dealloc_GPU_f("tridiag: u_col_dev ", 1745,  successGPU)

    successGPU = gpu_free(vu_stored_rows_dev)
    call check_dealloc_GPU_f("tridiag: vu_stored_rows_dev ", 1748,  successGPU)

    successGPU = gpu_free(uv_stored_cols_dev)
    call check_dealloc_GPU_f("tridiag:uv_stored_cols_dev ", 1751,  successGPU)
  endif ! useGPU

  ! distribute the arrays d_vec and e_vec to all processors

  allocate(tmp_real(na), stat=istat, errmsg=errorMessage)
  call check_allocate_f("tridiag: tmp_real", 1757,  istat,  errorMessage)
  ...
  call check_deallocate_f("tridiag: tmp_real", 1806,  istat,  errorMessage)

  if (useGPU) then

  else ! useGPU
    deallocate(v_row, v_col, u_row, u_col, stat=istat, errmsg=errorMessage)
    call check_deallocate_f("tridiag: v_row, v_col, u_row, u_col", 1861,  istat,  errorMessage)
  endif ! useGPU

  deallocate(vu_stored_rows, uv_stored_cols, stat=istat, errmsg=errorMessage)
  call check_deallocate_f("tridiag: vu_stored_rows, uv_stored_cols", 1865,  istat,  errorMessage)
...
!
  ...
  do istep = 1, na, blockStep
    ...
    if (nstor+nblk > max_stored_rows .or. istep+nblk > na .or. (na/np_rows <= 256 .and. nstor >= 32)) then
      ...
      if (useGPU) then
        ...
        call check_memcpy_GPU_f("trans_ev", 528,  successGPU)
        ...
        call check_memcpy_GPU_f("trans_ev", 533,  successGPU)
      endif
      ...
      if (l_rows > 0) then
        ...
        if (useGPU) then
          if (gpu_vendor() /= OPENMP_OFFLOAD_GPU) then
            ...
            call check_memcpy_GPU_f("trans_ev", 571,  successGPU)
          else
            ...
            call check_memcpy_GPU_f("trans_ev", 577,  successGPU)
      ...
      if (useGPU) then
        ...
        call check_memcpy_GPU_f("trans_ev", 613,  successGPU)
      ...
      if (useGPU) then
        ...
        call check_memcpy_GPU_f("trans_ev", 781,  successGPU)
      endif ! useGPU
  ...
  call check_deallocate_f("trans_ev_&     &MATH_DATATYPE&     &: h1, h2, hvb, hvm", 838,  istat,  errorMessage)
  ...
  if (useGPU) then
    ...
    call check_memcpy_GPU_f("trans_ev", 863,  successGPU)
    ...
    call check_dealloc_GPU_f("trans_ev", 905,  successGPU)

    successGPU = gpu_free(tmp_dev)
    call check_dealloc_GPU_f("trans_ev", 908,  successGPU)

    successGPU = gpu_free(hvm_dev)
    call check_dealloc_GPU_f("trans_ev", 911,  successGPU)

    successGPU = gpu_free(tmat_dev)
    call check_dealloc_GPU_f("trans_ev", 914,  successGPU)
  else ! useGPU
    deallocate(tmat, tmp1, tmp2, stat=istat, errmsg=errorMessage)
    call check_deallocate_f("trans_ev_&     &MATH_DATATYPE&     &: tmat, tmp1, tmp2", 919,  istat,  errorMessage)
  endif ! useGPU
      ...
      if (my_prow==prow(1, nblk, np_rows)) then
       ...
       if (useGPU) then
         successGPU = gpu_memcpy(int(loc(aux3(1)),kind=c_intptr_t), a_dev + (matrixRows * (l_cols - 1)) * size_of_datatype, &
                                 1 * size_of_datatype, gpuMemcpyDeviceToHost)
         call check_memcpy_GPU_f("tridiag: a_dev 5", 1597,  successGPU)
         vrl = aux3(1)
       else !useGPU
         vrl = a_mat(1,l_cols)
       endif !useGPU
  ...
  if (my_prow == prow(1, nblk, np_rows) .and. my_pcol == pcol(1, nblk, np_cols))  then
    if (useGPU) then
      successGPU = gpu_memcpy(int(loc(aux3(1)),kind=c_intptr_t), a_dev, &
                             1 * size_of_datatype, gpuMemcpyDeviceToHost)
      call check_memcpy_GPU_f("tridiag: a_dev 6", 1659,  successGPU)
