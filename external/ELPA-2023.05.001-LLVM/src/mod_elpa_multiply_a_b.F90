...
!cannot use "../src/multiply_a_b/../general/error_checking.inc" because filename with path can be too long for gfortran (max line length)
...
!
  ...
  if (useGPU) then
    ...
    call check_alloc_GPU_f("elpa_mult_at_b: b_dev", 259,  successGPU)

    successGPU = gpu_host_register(int(loc(b),kind=c_intptr_t),num,&
                  gpuHostRegisterDefault)

    call check_host_register_GPU_f("elpa_mult_at_b: b", 266,  successGPU)
    successGPU = gpu_memcpy(b_dev,int(loc(b),kind=c_intptr_t),num,&
                  gpuMemcpyHostToDevice)
    call check_memcpy_GPU_f("elpa_mult_at_b: b to b_dev", 285,  successGPU)

    num = l_rows*nblk_mult*size_of_datatype
    successGPU = gpu_malloc_host(aux_host, num)
    call check_host_alloc_GPU_f("elpa_mult_at_b: aux_host", 333,  successGPU)
    call c_f_pointer(aux_host, aux_mat, (/l_rows,nblk_mult/))

    successGPU = gpu_malloc(aux_dev,num)
    call check_alloc_GPU_f("elpa_mult_at_b: aux_dev", 340,  successGPU)

    num = nblk_mult*l_cols*size_of_datatype
    successGPU = gpu_malloc_host(tmp1_host,num)
    call check_host_alloc_GPU_f("elpa_mult_at_b: tmp1_host", 345,  successGPU)
    call c_f_pointer(tmp1_host,tmp1,(/nblk_mult,l_cols/))

    successGPU = gpu_malloc(tmp1_dev,num)
    call check_alloc_GPU_f("elpa_mult_at_b: tmp1_dev", 352,  successGPU)
  else ! useGPU
    allocate(aux_mat(l_rows,nblk_mult), stat=istat, errmsg=errorMessage)
    call check_allocate_f("elpa_mult_at_b: aux_mat", 355,  istat,  errorMessage)
  endif ! useGPU

  allocate(aux_bc(l_rows*nblk), stat=istat, errmsg=errorMessage)
  call check_allocate_f("elpa_mult_at_b: aux_bc", 359,  istat,  errorMessage)

  allocate(lrs_save(nblk), stat=istat, errmsg=errorMessage)
  call check_allocate_f("elpa_mult_at_b: lrs_save", 362,  istat,  errorMessage)

  allocate(lre_save(nblk), stat=istat, errmsg=errorMessage)
  call check_allocate_f("elpa_mult_at_b: lre_save", 365,  istat,  errorMessage)
        ...
        if (lcs<=lce) then
          ...
          if (lrs<=lre) then
            if (useGPU) then
              ...
              call check_memcpy_GPU_f("elpa_mult_at_b: aux_mat to aux_dev", 500,  successGPU)
              ...
              call check_memcpy_GPU_f("elpa_mult_at_b: tmp1_dev to tmp1", 532,  successGPU)
            else ! useGPU
          ...
          call check_deallocate_f("elpa_mult_at_b: tmp1, tmp2", 574,  istat,  errorMessage)
        endif
  ...
  if (useGPU) then
    ...
    call check_dealloc_GPU_f("elpa_multiply_a_b: b_dev", 587,  successGPU)
    successGPU = gpu_host_unregister(int(loc(b),kind=c_intptr_t))
    call check_host_unregister_GPU_f("elpa_multiply_a_b: b", 590,  successGPU)

    nullify(aux_mat)
    nullify(tmp1)

    successGPU = gpu_free_host(aux_host)
    call check_host_dealloc_GPU_f("elpa_multiply_a_b: aux_host", 636,  successGPU)

    successGPU = gpu_free_host(tmp1_host)
    call check_host_dealloc_GPU_f("elpa_multiply_a_b: tmp1_host", 639,  successGPU)

    successGPU = gpu_free(aux_dev)
    call check_dealloc_GPU_f("elpa_multiply_a_b: aux_dev", 646,  successGPU)

    successGPU = gpu_free(tmp1_dev)
    call check_dealloc_GPU_f("elpa_multiply_a_b: tmp1_dev", 649,  successGPU)
  else ! useGPU
    deallocate(aux_mat, stat=istat, errmsg=errorMessage)
    call check_deallocate_f("elpa_mult_at_b: aux_mat", 652,  istat,  errorMessage)
  endif ! useGPU

  deallocate(aux_bc, lrs_save, lre_save, stat=istat, errmsg=errorMessage)
  call check_deallocate_f("elpa_mult_at_b: aux_bc, lrs_save, lre_save", 656,  istat,  errorMessage)
...
!
  ...
  integer(kind=ik)                             :: na, ncb
  type(c_ptr)                                  :: a, b, c
  real(kind=rck), allocatable         :: a_tmp(:,:), c_tmp(:,:)
  ...
  if (useGPU) then
    ...
    call check_allocate_f("elpa_mult_at_b: a_tmp", 291,  istat,  errorMessage)

    num = obj%local_nrows*obj%local_ncols*size_of_datatype
    successGPU = gpu_memcpy(int(loc(a_tmp),kind=c_intptr_t), a, num,&
                  gpuMemcpyDeviceToHost)
    call check_memcpy_GPU_f("elpa_mult_at_b: a_dev -> a_tmp", 317,  successGPU)

    allocate(c_tmp(ldc,ldcCols), stat=istat, errmsg=errorMessage)
    call check_allocate_f("elpa_mult_at_b: c_tmp", 321,  istat,  errorMessage)
  ...
  if (useGPU) then
    ...
    call check_deallocate_f("elpa_mult_at_b: a_tmp", 598,  istat,  errorMessage)

    num = ldc*ldcCols*size_of_datatype
    successGPU = gpu_memcpy(c,int(loc(c_tmp),kind=c_intptr_t),num,&
                  gpuMemcpyHostToDevice)
    call check_memcpy_GPU_f("elpa_mult_at_b: c_tmp -> c", 619,  successGPU)


    deallocate(c_tmp, stat=istat, errmsg=errorMessage)
    call check_deallocate_f("elpa_mult_at_b: c_tmp", 628,  istat,  errorMessage)
...
!
  ...
  integer(kind=ik)                             :: na, ncb
  type(c_ptr)                                  :: a, b, c
  complex(kind=rck), allocatable         :: a_tmp(:,:), c_tmp(:,:)
