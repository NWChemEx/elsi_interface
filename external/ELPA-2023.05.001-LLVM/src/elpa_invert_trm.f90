...
!
  ...
  if (useGPU) then
    ...
    call check_alloc_GPU_f("elpa_invert_trm: tmp1_dev", 259,  successGPU)

    successGPU = gpu_memset(tmp1_dev, 0, nblk*nblk*size_of_datatype)
    call check_memcpy_GPU_f("elpa_invert_trm: memset tmp1_dev", 273,  successGPU)

    successGPU = gpu_malloc(tmp2_dev, nblk*nblk*size_of_datatype)
    call check_alloc_GPU_f("elpa_invert_trm: tmp2_dev", 277,  successGPU)

    successGPU = gpu_memset(tmp2_dev, 0, nblk*nblk*size_of_datatype)
    call check_memcpy_GPU_f("elpa_invert_trm: memset tmp2_dev", 291,  successGPU)

    successGPU = gpu_malloc(tmat1_dev, l_rows*nblk*size_of_datatype)
    call check_alloc_GPU_f("elpa_invert_trm: tmat1_dev", 295,  successGPU)

    successGPU = gpu_memset(tmat1_dev, 0, l_rows*nblk*size_of_datatype)
    call check_memcpy_GPU_f("elpa_invert_trm: memset tmat1_dev", 309,  successGPU)

    successGPU = gpu_malloc(tmat2_dev, nblk*l_cols*size_of_datatype)
    call check_alloc_GPU_f("elpa_invert_trm: tmat1_dev", 313,  successGPU)

    successGPU = gpu_memset(tmat2_dev, 0, nblk*l_cols*size_of_datatype)
    call check_memcpy_GPU_f("elpa_invert_trm: memset tmat2_dev", 327,  successGPU)

    successGPU = gpu_malloc(a_dev, matrixRows*matrixCols*size_of_datatype)
    call check_alloc_GPU_f("elpa_invert_trm: a_dev", 332,  successGPU)

    successGPU = gpu_malloc(zero_dev, 1*size_of_datatype)
    call check_alloc_GPU_f("elpa_invert_trm: zero_dev", 354,  successGPU)
    successGPU = gpu_memset(zero_dev, 0, 1*size_of_datatype)
    call check_memcpy_GPU_f("elpa_invert_trm: memset zero_dev", 367,  successGPU)
  endif ! useGPU


  allocate(tmp1(nblk*nblk), stat=istat, errmsg=errorMessage)
  call check_allocate_f("elpa_invert_trm: tmp1", 373,  istat,  errorMessage)

  allocate(tmp2(nblk,nblk), stat=istat, errmsg=errorMessage)
  call check_allocate_f("elpa_invert_trm: tmp2", 376,  istat,  errorMessage)

  tmp1 = 0
  tmp2 = 0

  allocate(tmat1(l_rows,nblk), stat=istat, errmsg=errorMessage)
  call check_allocate_f("elpa_invert_trm: tmat1", 382,  istat,  errorMessage)

  allocate(tmat2(nblk,l_cols), stat=istat, errmsg=errorMessage)
  call check_allocate_f("elpa_invert_trm: tmat2", 385,  istat,  errorMessage)
  ...
  if (useGPU) then
    ...
    call check_memcpy_GPU_f("elpa_invert_trm: memcpy a-> d_dev", 430,  successGPU)
  endif
    ...
    if (my_prow==prow(n, nblk, np_rows)) then
      ...
      if (my_pcol==pcol(n, nblk, np_cols)) then
        if (useGPU) then
          ...
          call check_memcpy_GPU_f("invert_trm: memcpy a_dev -> a", 488,  successGPU)
        ...
        if (useGPU) then
          ...
          call gpu_copy_double_a_tmp1 (a_dev, tmp1_dev, l_row1, l_col1, matrixRows, nb, my_stream)
        else ! useGPU
          nc = 0
          do i=1,nb
...
!#ifndef WITH_CUDA_AWARE_MPI
      if (useGPU) then
        ...
        call check_memcpy_GPU_f("elpa_invert_trm: tmp1_dev to tmp1", 634,  successGPU)
...
!      call check_memcpy_GPU_f("invert_trm: device_synchronize", 651,  successGPU)
...
!#ifndef WITH_CUDA_AWARE_MPI
      if ((useGPU)) then  
        ...
        call check_memcpy_GPU_f("elpa_invert_trm: tmp1 to tmp1_dev", 682,  successGPU)
      endif
!#endif
      
      if (useGPU) then
        call gpu_copy_double_tmp1_tmp2 (tmp1_dev, tmp2_dev, nblk, nb, my_stream)
      else ! useGPU
        nc = 0
        do i=1,nb
      ...
      if (useGPU) then
        ...
        if (l_colx <= l_cols) then
          my_stream = obj%gpu_setup%my_stream
          call gpu_copy_double_a_tmat2 (a_dev, tmat2_dev, nblk, matrixRows, l_cols, l_colx, & 
                                       l_row1, nb, my_stream)
        endif

        if (my_pcol==pcol(n, nblk, np_cols)) then
           ! tmp2 has the lower left triangle 0
          call gpu_copy_double_tmp2_tmat2 (tmp2_dev, tmat2_dev, nblk, l_col1, nb, my_stream) 
        endif
      else ! useGPU
        call obj%timer%start("blas")
        if (l_cols-l_colx+1>0) then
    ...
    if (l_row1>1) then
      if (my_pcol==pcol(n, nblk, np_cols)) then
        if (useGPU) then
          my_stream = obj%gpu_setup%my_stream
          call gpu_copy_double_a_tmat1 (a_dev, tmat1_dev, l_rows, matrixRows, nb, l_row1, l_col1, zero_dev, my_stream)
        else
...
!#ifndef WITH_CUDA_AWARE_MPI
      if (useGPU) then
        ...
        successGPU = gpu_memcpy(int(loc(tmat1),kind=c_intptr_t), tmat1_dev, num, &
                              gpuMemcpyDeviceToHost)
        call check_memcpy_GPU_f("elpa_invert_trm: tmat1_dev to tmat1", 780,  successGPU)
      endif
!#endif
...
!      if (wantDebug) call obj%timer%start("cuda_aware_device_synchronize")
!      successGPU = gpu_devicesynchronize()
!      call check_memcpy_GPU_f("invert_trm: device_synchronize", 802,  successGPU)
!      if (wantDebug) call obj%timer%stop("cuda_aware_device_synchronize")
...
!#ifndef WITH_CUDA_AWARE_MPI
      if (useGPU) then
        ...
        successGPU = gpu_memcpy(tmat1_dev, int(loc(tmat1),kind=c_intptr_t), num, &
                              gpuMemcpyHostToDevice)
        call check_memcpy_GPU_f("elpa_invert_trm: tmat1 to tmat1_dev", 837,  successGPU)
      endif
!#endif
    endif ! (l_row1>1)
    ...
    if (useGPU) then
      ...
      if (l_cols-l_col1+1 > 0) then
        ...
        successGPU = gpu_memcpy(int(loc(tmat2),kind=c_intptr_t), tmat2_dev, num, &
                              gpuMemcpyDeviceToHost)
        call check_memcpy_GPU_f("elpa_invert_trm: tmat2_dev to tmat2", 867,  successGPU)
      endif
    endif
    ...
    if (useGPU) then
      if (l_cols-l_col1+1 > 0) then
        ...
        successGPU = gpu_memcpy(tmat2_dev, int(loc(tmat2),kind=c_intptr_t), num, &
                                gpuMemcpyHostToDevice)
        call check_memcpy_GPU_f("elpa_invert_trm: tmat2 to tmat2_dev", 900,  successGPU)
      endif
    endif
!#else
...
!      if (wantDebug) call obj%timer%start("cuda_aware_device_synchronize")
!      successGPU = gpu_devicesynchronize()
!      call check_memcpy_GPU_f("invert_trm: device_synchronize", 910,  successGPU)
!      if (wantDebug) call obj%timer%stop("cuda_aware_device_synchronize")
!      call obj%timer%start("mpi_cuda_communication")
...
!#endif
  ...
  ! copy results back
    successGPU = gpu_memcpy(int(loc(a(1,1)),kind=c_intptr_t), a_dev,  &
                       matrixRows*matrixCols* size_of_datatype, gpuMemcpyDeviceToHost)
    call check_memcpy_GPU_f("elpa_invert_trm: memcpy a-> d_dev", 979,  successGPU)
  endif

  if (useGPU) then
    successGPU = gpu_free(tmp1_dev)
    call check_dealloc_GPU_f("elpa_invert_trm: tmp1_dev", 986,  successGPU)

    successGPU = gpu_free(tmp2_dev)
    call check_dealloc_GPU_f("elpa_invert_trm: tmp2_dev", 989,  successGPU)

    successGPU = gpu_free(tmat1_dev)
    call check_dealloc_GPU_f("elpa_invert_trm: tmat1_dev", 992,  successGPU)

    successGPU = gpu_free(tmat2_dev)
    call check_dealloc_GPU_f("elpa_invert_trm: tmat2_dev", 995,  successGPU)

    successGPU = gpu_free(a_dev)
    call check_dealloc_GPU_f("elpa_invert_trm: a_dev", 999,  successGPU)



    successGPU = gpu_free(zero_dev)
    call check_dealloc_GPU_f("elpa_invert_trm: zero_dev", 1017,  successGPU)

    !successGPU = gpu_host_unregister(int(loc(b),kind=c_intptr_t))
    !call check_host_unregister_GPU_f("elpa_multiply_a_b: b", 1020,  successGPU)
  endif ! useGPU


  deallocate(tmp1, tmp2, tmat1, tmat2, stat=istat, errmsg=errorMessage)
  call check_deallocate_f("elpa_invert_trm: tmp1, tmp2, tmat1, tmat2", 1037,  istat,  errorMessage)
...
!
  ...
  if (useGPU) then
    ...
    call check_allocate_f("elpa_invert_trm: a_tmp", 344,  istat,  errorMessage)
      ...
      if (my_pcol==pcol(n, nblk, np_cols)) then
        if (useGPU) then
          ...
          successGPU = gpu_memcpy(int(loc(a_tmp(1,1)),kind=c_intptr_t), a_dev, &
                       matrixRows*matrixCols* size_of_datatype, gpuMemcpyDeviceToHost)
          call check_memcpy_GPU_f("invert_trm: memcpy a_dev -> a", 534,  successGPU)
          ...
          successGPU = gpu_memcpy(a_dev, int(loc(a_tmp(1,1)),kind=c_intptr_t),  &
                       matrixRows*matrixCols* size_of_datatype, gpuMemcpyHostToDevice)
          call check_memcpy_GPU_f("invert_trm: memcpy a -> a_dev", 558,  successGPU)
          call obj%timer%stop("blas")
...
!#endif
  ...
  if (useGPU) then
    ...
    call check_deallocate_f("elpa_invert_trm: a_tmp", 1013,  istat,  errorMessage)
  ...
  do n = ns,1,-nblk
    ...
    if (my_prow==prow(n, nblk, np_rows)) then
      ...
      if (my_pcol==pcol(n, nblk, np_cols)) then
        ...
        if (useGPU) then
          my_stream = obj%gpu_setup%my_stream
          call gpu_copy_float_a_tmp1 (a_dev, tmp1_dev, l_row1, l_col1, matrixRows, nb, my_stream)
        else ! useGPU
          nc = 0
          do i=1,nb
            tmp1(nc+1:nc+i) = a(l_row1:l_row1+i-1,l_col1+i-1)
...
!#endif
      ...
      if (useGPU) then
        call gpu_copy_float_tmp1_tmp2 (tmp1_dev, tmp2_dev, nblk, nb, my_stream)
      else ! useGPU
        nc = 0
        do i=1,nb
          tmp2(1:i,i) = tmp1(nc+1:nc+i)
      ...
      if (useGPU) then
        ...
        if (l_colx <= l_cols) then
          my_stream = obj%gpu_setup%my_stream
          call gpu_copy_float_a_tmat2 (a_dev, tmat2_dev, nblk, matrixRows, l_cols, l_colx, & 
                                       l_row1, nb, my_stream)
        endif

        if (my_pcol==pcol(n, nblk, np_cols)) then
           ! tmp2 has the lower left triangle 0
          call gpu_copy_float_tmp2_tmat2 (tmp2_dev, tmat2_dev, nblk, l_col1, nb, my_stream) 
        endif
      else ! useGPU
        call obj%timer%start("blas")
        if (l_cols-l_colx+1>0) then
    ...
    if (l_row1>1) then
      if (my_pcol==pcol(n, nblk, np_cols)) then
        if (useGPU) then
          my_stream = obj%gpu_setup%my_stream
          call gpu_copy_float_a_tmat1 (a_dev, tmat1_dev, l_rows, matrixRows, nb, l_row1, l_col1, zero_dev, my_stream)
        else
          tmat1(1:l_row1-1,1:nb) = a(1:l_row1-1,l_col1:l_col1+nb-1)
          a(1:l_row1-1,l_col1:l_col1+nb-1) = 0
        endif
  ...
  do n = ns,1,-nblk
    ...
    if (my_prow==prow(n, nblk, np_rows)) then
      ...
      if (my_pcol==pcol(n, nblk, np_cols)) then
        ...
        if (useGPU) then
          my_stream = obj%gpu_setup%my_stream
          call gpu_copy_double_complex_a_tmp1 (a_dev, tmp1_dev, l_row1, l_col1, matrixRows, nb, my_stream)
        else ! useGPU
          nc = 0
          do i=1,nb
            tmp1(nc+1:nc+i) = a(l_row1:l_row1+i-1,l_col1+i-1)
...
!#endif
      ...
      if (useGPU) then
        call gpu_copy_double_complex_tmp1_tmp2 (tmp1_dev, tmp2_dev, nblk, nb, my_stream)
      else ! useGPU
        nc = 0
        do i=1,nb
          tmp2(1:i,i) = tmp1(nc+1:nc+i)
      ...
      if (useGPU) then
        ...
        if (l_colx <= l_cols) then
          my_stream = obj%gpu_setup%my_stream
          call gpu_copy_double_complex_a_tmat2 (a_dev, tmat2_dev, nblk, matrixRows, l_cols, l_colx, & 
                                       l_row1, nb, my_stream)
        endif

        if (my_pcol==pcol(n, nblk, np_cols)) then
           ! tmp2 has the lower left triangle 0
          call gpu_copy_double_complex_tmp2_tmat2 (tmp2_dev, tmat2_dev, nblk, l_col1, nb, my_stream) 
        endif
      else ! useGPU
        call obj%timer%start("blas")
        if (l_cols-l_colx+1>0) then
    ...
    if (l_row1>1) then
      if (my_pcol==pcol(n, nblk, np_cols)) then
        if (useGPU) then
          my_stream = obj%gpu_setup%my_stream
          call gpu_copy_double_complex_a_tmat1 (a_dev, tmat1_dev, l_rows, matrixRows, nb, l_row1, l_col1, zero_dev, my_stream)
        else
          tmat1(1:l_row1-1,1:nb) = a(1:l_row1-1,l_col1:l_col1+nb-1)
          a(1:l_row1-1,l_col1:l_col1+nb-1) = 0
        endif
  ...
  do n = ns,1,-nblk
    ...
    if (my_prow==prow(n, nblk, np_rows)) then
      ...
      if (my_pcol==pcol(n, nblk, np_cols)) then
        ...
        if (useGPU) then
          my_stream = obj%gpu_setup%my_stream
          call gpu_copy_float_complex_a_tmp1 (a_dev, tmp1_dev, l_row1, l_col1, matrixRows, nb, my_stream)
        else ! useGPU
          nc = 0
          do i=1,nb
            tmp1(nc+1:nc+i) = a(l_row1:l_row1+i-1,l_col1+i-1)
...
!#endif
      ...
      if (useGPU) then
        call gpu_copy_float_complex_tmp1_tmp2 (tmp1_dev, tmp2_dev, nblk, nb, my_stream)
      else ! useGPU
        nc = 0
        do i=1,nb
          tmp2(1:i,i) = tmp1(nc+1:nc+i)
      ...
      if (useGPU) then
        ...
        if (l_colx <= l_cols) then
          my_stream = obj%gpu_setup%my_stream
          call gpu_copy_float_complex_a_tmat2 (a_dev, tmat2_dev, nblk, matrixRows, l_cols, l_colx, & 
                                       l_row1, nb, my_stream)
        endif

        if (my_pcol==pcol(n, nblk, np_cols)) then
           ! tmp2 has the lower left triangle 0
          call gpu_copy_float_complex_tmp2_tmat2 (tmp2_dev, tmat2_dev, nblk, l_col1, nb, my_stream) 
        endif
      else ! useGPU
        call obj%timer%start("blas")
        if (l_cols-l_colx+1>0) then
    ...
    if (l_row1>1) then
      if (my_pcol==pcol(n, nblk, np_cols)) then
        if (useGPU) then
          my_stream = obj%gpu_setup%my_stream
          call gpu_copy_float_complex_a_tmat1 (a_dev, tmat1_dev, l_rows, matrixRows, nb, l_row1, l_col1, zero_dev, my_stream)
        else
          tmat1(1:l_row1-1,1:nb) = a(1:l_row1-1,l_col1:l_col1+nb-1)
          a(1:l_row1-1,l_col1:l_col1+nb-1) = 0
        endif
